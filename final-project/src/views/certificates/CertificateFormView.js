 ...
 */
const CustomSelect = ({ label, value, options = [], placeholder = 'Select...', onSelect }) => {
    ...
    return (
        <>
            ...
            >
                <Text style={[styles.selectText, !selectedLabel && { color: '#9CA3AF' }]}>
...
const CertificateFormView = ({ certificateId, onBack, onSaved }) => {
    ...
    const courseOptions = mockCourses.map(c => ({ label: c.title, value: String(c.id) }));
    ...
    useEffect(() => {
        if (isEditMode) {
            ...
            if (certificate) {
                ...
                Alert.alert('Not found', 'Certificate to edit was not found', [{ text: 'OK', onPress: onBack }]);
            }
        } else {
            // reset for create
            setFormData({
    ...
    const handleSubmit = () => {
        ...
        if (isEditMode) {
            const updated = CertificateModel.updateCertificate(Number(certificateId), data);
            if (updated) {
    ...
    const setToday = () => {
        setFormData({ ...formData, issueDate: new Date().toISOString().split('T')[0] });
    };
        ...
        <SafeAreaView style={styles.container}>
            ...
            >
                ...
                <View style={styles.header}>
                    ...
                    <Text style={styles.headerTitle}>{isEditMode ? 'Edit Certificate' : 'Create Certificate'}</Text>
                ...
                >
                    <View style={styles.card}>
                        ...
                        <View style={styles.formRow}>
                            ...
                            <TextInput
                                style={[styles.input, errors.certificateName && styles.inputError]}
                            ...
                            />
                            {errors.certificateName ? <Text style={styles.errorText}>{errors.certificateName}</Text> : null}
                        </View>
                        ...
                        <View style={styles.formRow}>
                            ...
                            <CustomSelect
                                ...
                                onSelect={(val) => setFormData({ ...formData, validityPeriod: val })}
                            />
                        </View>
                        ...
                        <View style={styles.formRow}>
                            ...
                            <CustomSelect
                                ...
                                onSelect={(val) => setFormData({ ...formData, templateDesign: val })}
                            />
                        </View>
                        ...
                        <View style={styles.formRowRow}>
                            <View style={{ flex: 1 }}>
                                <Text style={styles.label}>Issue Date {isEditMode ? '(read-only)' : '*'}</Text>
                                <TextInput
                                    style={[styles.input, isEditMode ? styles.inputDisabled : null]}
                                    value={formData.issueDate}
                                    onChangeText={(text) => !isEditMode && setFormData({ ...formData, issueDate: text })}
                                    editable={!isEditMode}
                        ...
                        {isEditMode && (
                            <View style={styles.formRow}>
                                ...
                                <View style={styles.statusRow}>
                                    <Text style={[
                                        styles.statusLabel,
                                        formData.status === 'active' ? styles.statusLabelActive : styles.statusLabelInactive
                                    ]}>
                                    ...
                                    <Switch
                                        ...
                                        thumbColor={Platform.OS === 'android' ? (formData.status === 'active' ? '#16A34A' : '#DC2626') : undefined}
                        ...
                        <View style={styles.actionsWrap}>
                            <TouchableOpacity style={styles.submitButton} onPress={handleSubmit}>
                                ...
                                <Text style={styles.submitText}>{isEditMode ? 'Update Certificate' : 'Create Certificate'}</Text>
                            </TouchableOpacity>
