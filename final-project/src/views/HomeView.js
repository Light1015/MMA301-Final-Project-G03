...
export default function HomeView({
  ...
  const handleBackFromCourseManagement = () => {
    setShowCourseManagement(false);
  };

  const handleNavigateToCourseCatalog = () => {
    setShowCourseCatalog(true);
  };

  const handleBackFromCourseCatalog = () => {
    setShowCourseCatalog(false);
  };

  // Show Course Management View
  if (showCourseManagement) {
    return (
      <CourseManagementView
        user={user}
        onBack={handleBackFromCourseManagement}
      />
    );
  }

  // Show Course Catalog View
  if (showCourseCatalog) {
    return (
      <CourseCatalogView user={user} onBack={handleBackFromCourseCatalog} />
    );
  }
